...
def map_view():
    with get_db() as db:
        # All projects (for filter dropdowns + total count)
        ...
        # Projects without coordinates (listed in sidebar)
    ...
    for r in mapped:
        d = row_to_dict(r)
        # Fall back to first image if no hero set
        if not d.get('hero_file'):
            with get_db() as db2:
                first = db2.execute(
                    f'SELECT filename FROM images WHERE project_id = {P} ORDER BY id LIMIT 1',
...
def save_project(project_id):
    ...
    if new_address and new_address != old_address:
        ...
        # Address unchanged – keep existing coords
        with get_db() as db:
