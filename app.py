...
def export_resume(employee_id):
    ...
    # ── Colour palette ──────────────────────────────────────────────────────────
    GOLD     = HexColor('#B59030')   # first name, section labels
    MAROON   = HexColor('#8D0134')   # title, sidebar panel, logo
    ...
    def _draw_sidebar(cv):
        ...
        def _section(label, draw_body_fn):
            nonlocal y
            _spaced(cv, label, RT_X, y, 'Helvetica-Bold', 9, GOLD, tracking=1.2)
            y -= 7
    ...
    def draw_page(cv, doc_obj):
        ...
        if pg == 1:
            ...
            # ── Fontaine logo: 3 stacked maroon rectangles (template-measured) ─
            cv.setFillColor(MAROON)
            cv.rect(571.6, H - 23.1,  25.9, 9.1, fill=1, stroke=0)  # widest / top
            cv.rect(571.6, H - 34.8,  20.4, 9.0, fill=1, stroke=0)  # middle
            cv.rect(571.6, H - 46.5,   9.0, 9.0, fill=1, stroke=0)  # narrowest / bottom

            # ── Left column: name block ───────────────────────────────────────
            # First name — large bold gold
            cv.setFont('Helvetica-Bold', 30)
            cv.setFillColor(GOLD)
            cv.drawString(LT_MARG, H - 72, first_name)

            # Last name — large regular dark
            cv.setFont('Helvetica', 25)
            cv.setFillColor(DARK)
            cv.drawString(LT_MARG, H - 99, last_name)

            # Title — spaced caps, maroon
            if title_txt:
                _spaced(cv, title_txt, LT_MARG + 2, H - 113,
                        'Helvetica-Bold', 10, MAROON, tracking=1.5)
    ...
    class SpacedHeading(Flowable):
        """Draws a letter-spaced gold label with a gray rule below it."""
    ...
    pnm_st  = ps('PNm',  fontSize=9,   fontName='Helvetica-Bold',
                 textColor=DARK,  spaceBefore=8, spaceAfter=2)
    pds_st  = ps('PDs',  fontSize=8.5, fontName='Helvetica',
