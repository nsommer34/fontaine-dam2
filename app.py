...
def export_resume(employee_id):
    ...
    def draw_page(cv, doc_obj):
        ...
        if pg == 1:
            ...
            # Last name — thin serif for elegant contrast
            _last_font = 'SerifReg' if _HAS_SERIF else 'Times-Roman'
            cv.setFont(_last_font, 26)
            cv.setFillColor(DARK)
            cv.drawString(LT_MARG, H - 100, last_name)

            # Title — spaced caps, maroon, extra-heavy
            if title_txt:
                cv.setLineWidth(0.35)
                t2 = cv.beginText(0, 0)   # dummy; we use _spaced with tObj below
                # draw spaced text manually with fill+stroke
                cv.setFont('Helvetica-Bold', 10)
                cv.setFillColor(MAROON)
                cv.setStrokeColor(MAROON)
                x_t = LT_MARG + 2
                y_t = H - 114
                for ch in title_txt:
                    t3 = cv.beginText(x_t, y_t)
                    t3.setFont('Helvetica-Bold', 10)
                    t3.setFillColor(MAROON)
                    t3.setStrokeColor(MAROON)
    ...
    bio_st  = ps('Bio',  fontSize=9.5, fontName='Helvetica-Bold',
