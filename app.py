...
def export_resume(employee_id):
    ...
    try:
        ...
        pdfmetrics.registerFont(TTFont('SansNarrow',  str(_FONT_DIR / 'LiberationSansNarrow-Regular.ttf')))
        _HAS_SERIF = True
    except Exception:
        _HAS_SERIF = False  # fall back to built-ins if fonts missing
    ...
    def draw_page(cv, doc_obj):
        ...
        if pg == 1:
            ...
            # Last name — narrow condensed sans, black, fill-only (thinnest available)
            _last_font = 'SansNarrow' if _HAS_SERIF else 'Helvetica'
            t_last = cv.beginText(LT_MARG, H - 93)
