...
def export_resume(employee_id):
    ...
    def _ensure_light_font():
        """Download Inter Light (300) from Google Fonts and cache it locally.
        Returns the file path on success, None on failure."""
        dest = _FONT_DIR / 'InterLight.ttf'
        if dest.exists():
            return str(dest)
        try:
            import requests as _req, re as _re
            # Old CSS1 API returns TTF URLs for legacy user-agents
            hdrs = {'User-Agent': 'Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)'}
            css = _req.get('https://fonts.googleapis.com/css?family=Inter:300',
    ...
    _light_path = _ensure_light_font()
    try:
        if _light_path:
            pdfmetrics.registerFont(TTFont('SansLight', _light_path))
            _HAS_LIGHT = True
        else:
            _HAS_LIGHT = False
    except Exception:
    ...
    def draw_page(cv, doc_obj):
        ...
        if pg == 1:
            ...
            # Headshot — fitted, anchored to top so face is visible
            if headshot_path:
                try:
                    cv.saveState()
                    clip = cv.beginPath()
                    clip.rect(RT_COL_X, PHOTO_BOT, W - RT_COL_X, PHOTO_H)
                    cv.clipPath(clip, fill=0, stroke=0)
                    cv.drawImage(headshot_path,
                                 RT_COL_X, PHOTO_BOT,
                                 width=W - RT_COL_X, height=PHOTO_H,
            ...
            # First name — extra-heavy gold (fill + stroke via textObject)
            cv.setLineWidth(0.55)
            t = cv.beginText(LT_MARG, H - 72)
            t.setFont('Helvetica-Bold', 30)
            t.setFillColor(GOLD)
            t.setStrokeColor(GOLD)
            t.setTextRenderMode(2)   # fill + stroke = heavier appearance
            t.textLine(first_name)
            cv.drawText(t)
            cv.setLineWidth(1)
            cv._code.append('0 Tr')  # reset text render mode — prevents stroke leaking

            # Last name — Light-weight sans (Inter Light), black, fill-only
            _last_font = 'SansLight' if _HAS_LIGHT else 'Helvetica'
            t_last = cv.beginText(LT_MARG, H - 98)
            t_last.setFont(_last_font, 25)
            t_last.setFillColor(DARK)
            t_last.setTextRenderMode(0)   # explicit fill-only, no stroke
