...
def export_resume(employee_id):
    ...
    try:
        ...
        _HAS_SERIF = False  # fall back to Times if fonts missing
    ...
    def draw_page(cv, doc_obj):
        ...
        if pg == 1:
            ...
            # Last name — Helvetica regular, black, thinnest available (fill-only, no stroke)
            t_last = cv.beginText(LT_MARG, H - 100)
            t_last.setFont('Helvetica', 25)
            t_last.setFillColor(DARK)
            t_last.setTextRenderMode(0)   # explicit fill-only
        ...
        # ── Footer (all pages) ────────────────────────────────────────────────
        cv.setStrokeColor(MED_GRAY)
        cv.setLineWidth(0.4)
        cv.line(LT_MARG, BOT_MARG - 10, RT_COL_X - 12, BOT_MARG - 10)
        cv.setFont('Helvetica', 7.5)
        cv.setFillColor(MED_GRAY)
        cv.drawString(LT_MARG, BOT_MARG - 20,
                      'Fontaine Bros. Construction Management')
        cv.drawRightString(RT_COL_X - 12, BOT_MARG - 20,
                           f'Page {pg}')
        cv.restoreState()
