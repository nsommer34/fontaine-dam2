...
def _parse_resume_text(text):
    ...
    resume text (PDF-extracted). Uses section-header detection to split the
    document into labelled chunks, then assigns each chunk to the right field.

    Returns a dict with keys: name, title, bio, education, affiliations, ref_info.
    """
    ...
    # ── Contact-info detector (skip for name/title) ──────────────────────────
    contact_re = re.compile(
        r'(\bphone\b|\bemail\b|\blinkedin\b|@|\d{3}[-.\s]?\d{3}[-.\s]?\d{4}'
        r'|\bwww\b|http|\.com\b|\.edu\b|\.org\b)',
        re.I
    )
    ...
    SECTION_MAP = {
        # bio / summary
        'bio':          re.compile(
            r'^(summary|professional summary|profile|about me|objective|overview'
            r'|career summary|executive summary|qualifications)',
            re.I),
        # education
        'education':    re.compile(
            r'^(education|academic|degrees?|credentials?|schooling|training'
            r'|certifications?|licenses?|licensure)',
            re.I),
        # affiliations / memberships
        'affiliations': re.compile(
            r'^(affiliations?|memberships?|professional (memberships?|affiliations?'
            r'|organizations?|associations?)|organizations?|associations?'
            r'|civic|volunteer|community)',
            re.I),
        # references
        'ref_info':     re.compile(
            r'^(references?|professional references?)',
            re.I),
        # sections we collect but don't map to a specific field (skip content)
        'skip':         re.compile(
            r'^(experience|work experience|employment|professional experience'
            r'|career history|projects?|skills?|technical skills?|publications?'
            r'|awards?|honors?|achievements?|activities|leadership|languages?'
            r'|certifications? and (training|licenses?)|publications? and presentations?)',
            re.I),
    }

    def _is_section_header(line):
        """Return (field_key | 'skip' | None) for a line."""
        stripped = line.rstrip(':').strip()
        # Section headers are always short standalone labels (≤ 55 chars).
        # This prevents body-text sentences that happen to start with a
        # keyword word (e.g. "Experienced…" matching "experience") from
        # being mis-classified as headers.
        if len(stripped) > 55:
            return None
        for key, pattern in SECTION_MAP.items():
            m = pattern.match(stripped)
            # Require the pattern to consume the whole stripped string so that
            # "Experience: 10 years" isn't a false positive for "experience"
            if m and m.end() >= len(stripped):
                return key
        # Treat short ALL-CAPS lines as generic section headers (→ skip)
        if line.isupper() and len(line) < 60:
            return 'skip'
        return None

    # ── Pass 1: identify section boundaries ──────────────────────────────────
    sections = []   # list of (field_key, [content_lines])
    current_key   = None
    current_lines = []
    ...
    # ── Pass 2: extract name & title from pre-section lines ──────────────────
    pre_lines = []
    if sections and sections[0][0] is not None:
        # lines before the first recognised header are in current_lines at start
        pass
    # Re-scan: everything before the first section header
    first_header_idx = None
    for i, line in enumerate(lines):
        if _is_section_header(line) is not None:
            first_header_idx = i
            break
    header_block = lines[:first_header_idx] if first_header_idx else lines[:8]
    ...
    # ── Pass 3: bucket section content into fields ───────────────────────────
    buckets = {'bio': [], 'education': [], 'affiliations': [], 'ref_info': []}

    for key, content in sections:
        if key == 'skip' or key is None:
            continue
        # Filter obvious contact lines from content
        clean = [l for l in content if not contact_re.search(l)]
        buckets[key].extend(clean)

    def _join(lines_list, max_chars=1200):
