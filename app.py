...
def export_resume(employee_id):
    ...
    def _ensure_montserrat(weight):
        """Download a specific Montserrat weight TTF from Google Fonts and cache it.
        weight: int, e.g. 900 for Black, 200 for ExtraLight.
        Returns the file path on success, None on failure."""
        dest = _FONT_DIR / f'Montserrat-{weight}.ttf'
        if dest.exists():
            return str(dest)
        try:
            import requests as _req, re as _re
            # Legacy CSS1 user-agent forces Google Fonts to return plain TTF URLs
            hdrs = {'User-Agent': 'Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)'}
            css = _req.get(
                f'https://fonts.googleapis.com/css?family=Montserrat:{weight}',
                headers=hdrs, timeout=8).text
            match = _re.search(r'url\((https://fonts\.gstatic\.com/[^)]+\.ttf)\)', css)
            if match:
                data = _req.get(match.group(1), headers=hdrs, timeout=15).content
    ...
    # Montserrat Black 900 — first name
    _black_path = _ensure_montserrat(900)
    try:
        if _black_path:
            pdfmetrics.registerFont(TTFont('MontserratBlack', _black_path))
            _HAS_BLACK = True
        else:
            _HAS_BLACK = False
    except Exception:
    ...
    # Montserrat ExtraLight 200 — last name
    _xlight_path = _ensure_montserrat(200)
    try:
        if _xlight_path:
            pdfmetrics.registerFont(TTFont('MontserratXLight', _xlight_path))
            _HAS_XLIGHT = True
        else:
            _HAS_XLIGHT = False
    except Exception:
    ...
    def _draw_sidebar(cv):
        ...
        def _section(label, draw_body_fn):
            nonlocal y
            # Sidebar title — extra-heavy via fill+stroke textObject per char
            cv.setLineWidth(0.4)
            cx = RT_X
            for ch in label:
                t = cv.beginText(cx, y)
                ...
                t.setFillColor(GOLD)
                t.setStrokeColor(GOLD)
                t.setTextRenderMode(2)
                t.textLine(ch)
                cv.drawText(t)
                cx += cv.stringWidth(ch, 'Helvetica-Bold', 9) + 1.2
            cv.setLineWidth(1)
            cv._code.append('0 Tr')  # reset render mode so body text isn't stroked
            y -= 7
            ...
            def _edu():
                ...
                for ln in edu:
                    y = _wrap(cv, ln, RT_X, y, RT_W,
                              'Helvetica', 8.5, RL_WHITE, leading=11)
    ...
    def draw_page(cv, doc_obj):
        ...
        if pg == 1:
            ...
            # First name — Montserrat Black 900, gold, fill-only (weight carries itself)
            _first_font = 'MontserratBlack' if _HAS_BLACK else 'Helvetica-Bold'
            t = cv.beginText(LT_MARG, H - 72)
            t.setFont(_first_font, 30)
            t.setFillColor(GOLD)
            t.setTextRenderMode(0)   # fill-only — Black 900 is heavy enough
            t.textLine(first_name)
            cv.drawText(t)
            cv._code.append('0 Tr')

            # Last name — Montserrat ExtraLight 200, dark, fill-only
            _last_font = 'MontserratXLight' if _HAS_XLIGHT else 'Helvetica'
            t_last = cv.beginText(LT_MARG, H - 98)
            t_last.setFont(_last_font, 25)
            t_last.setFillColor(DARK)
            t_last.setTextRenderMode(0)   # fill-only, no stroke
            t_last.textLine(last_name)
            cv.drawText(t_last)
            cv._code.append('0 Tr')

            # Title — spaced caps, black, extra-heavy (fill+stroke)
            if title_txt:
                ...
                for ch in title_txt:
                    ...
                    t3.setFont('Helvetica-Bold', 10)
                    t3.setFillColor(TITLE_BLACK)
                    t3.setStrokeColor(TITLE_BLACK)
                    t3.setTextRenderMode(2)
                    t3.textLine(ch)
                    cv.drawText(t3)
                    x_t += cv.stringWidth(ch, 'Helvetica-Bold', 10) + 1.5
                cv.setLineWidth(1)
                cv._code.append('0 Tr')  # reset after stroked title
    ...
    class SpacedHeading(Flowable):
        ...
        def draw(self):
            ...
            for ch in self.text:
                ...
                t.setFillColor(GOLD)
                t.setStrokeColor(GOLD)
                t.setTextRenderMode(2)   # fill + stroke = extra bold
                t.textLine(ch)
                c.drawText(t)
                x += c.stringWidth(ch, 'Helvetica-Bold', 9) + 1.2
            c._code.append('0 Tr')  # reset so rule + subsequent text aren't stroked
    ...
    _slab_bold = 'SerifBold' if _HAS_SERIF else 'Times-Bold'
    _slab_reg  = 'SerifReg'  if _HAS_SERIF else 'Times-Roman'

    _bio_font = _slab_reg   # slab-serif feel for bio body text
    bio_st  = ps('Bio',  fontSize=9.5, fontName=_bio_font,
                 textColor=DARK,  leading=11, spaceAfter=2)
    pnm_st  = ps('PNm',  fontSize=9,   fontName=_slab_bold,
                 textColor=DARK,  spaceBefore=8, spaceAfter=2)
    pds_st  = ps('PDs',  fontSize=8.5, fontName=_slab_reg,
