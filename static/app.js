...
async function saveProject() {
  ...
  try {
    const res = await apiPost(`/api/project/${PROJECT_ID}/save`, data);

    // Update read-only view fields
    document.querySelectorAll('.field-edit').forEach(el => {
      const field = el.closest('.info-field');
      if (!field) return;
      const view = field.querySelector('.field-view');
      ...
      if (el.name === 'address') {
        if (view) {
          ...
          if (addr) {
            ...
            const mapped  = res.geocoded
              ? `<span class="geocoded-badge">✓ Mapped</span>` : '';
            view.innerHTML = `<a href="${mapsUrl}" target="_blank" class="address-link">📍 ${esc(addr)}</a>${mapped}`;
          } else {
            view.textContent = '—';
          }
        }
    ...
    const geoMsg = res.geocoded ? ' — address mapped 📍' : (data.address && !res.geocoded && data.address.trim() ? ' — address not found' : '');
    showToast(`✅ Project saved!${geoMsg}`, 'success');
  } catch (err) {
    showToast('❌ ' + err.message, 'error');
  } finally {
    btn.disabled    = false;
...
async function addPerson(cat) {
  ...
  try {
    ...
    const listId  = 'engineers-list';
    const emptyId = 'engineers-empty';
...
// ================================================================
//  PEOPLE — Team Members (employee picker)
// ================================================================
let _allEmployees = null;

async function openEmpPicker() {
  const panel = document.getElementById('emp-picker');
  panel.style.display = 'block';
  document.getElementById('empPickerSearch').value = '';
  document.getElementById('empPickerSearch').focus();
  ...
  renderEmpPicker(_allEmployees);
}

function closeEmpPicker() {
  document.getElementById('emp-picker').style.display = 'none';
}

function filterEmpPicker(q) {
  if (!_allEmployees) return;
  const filtered = q
    ? _allEmployees.filter(e =>
        e.name.toLowerCase().includes(q.toLowerCase()) ||
        (e.title || '').toLowerCase().includes(q.toLowerCase()))
    : _allEmployees;
  renderEmpPicker(filtered);
}

function renderEmpPicker(employees) {
  const list = document.getElementById('empPickerList');
  if (!employees.length) {
    list.innerHTML = `<p class="people-empty" style="grid-column:1/-1">
      No employees found. <a href="/employees">Add team members</a> first.
    </p>`;
    return;
  }
  list.innerHTML = employees.map(e => {
    const avatar = e.headshot_filename
      ? `<img src="/headshots/${e.headshot_filename}" alt="${esc(e.name)}" class="emp-picker-avatar" />`
      : `<div class="emp-picker-avatar-placeholder">${esc(e.name[0].toUpperCase())}</div>`;
    return `<button class="emp-picker-card" onclick="addTeamMember(${e.id})">
      ${avatar}
      <span class="emp-picker-name">${esc(e.name)}</span>
      ${e.title ? `<span class="emp-picker-title">${esc(e.title)}</span>` : ''}
    </button>`;
  }).join('');
}

async function addTeamMember(employeeId) {
  try {
    const res = await apiPost(`/api/project/${PROJECT_ID}/people/add`,
                              { category: 'team_member', employee_id: employeeId });

    const list = document.getElementById('team_member-list');
    document.getElementById('team_member-empty')?.remove();

    const avatarHtml = res.headshot_url
      ? `<img src="${res.headshot_url}" alt="${esc(res.name)}" class="person-avatar" />`
      : `<div class="person-avatar person-avatar-placeholder">${esc(res.name[0].toUpperCase())}</div>`;

    const profileLink = res.employee_id
      ? `<a href="/employee/${res.employee_id}" class="person-profile-link" title="View profile" onclick="event.stopPropagation()">↗</a>`
      : '';
    ...
    chip.innerHTML  = `
      ${avatarHtml}
      <div class="person-info">
        <span class="person-name">${esc(res.name)}${profileLink}</span>
        ${res.role ? `<span class="person-role">${esc(res.role)}</span>` : ''}
      </div>
      <button class="person-remove" onclick="removePerson(${res.id},this)" title="Remove">✕</button>`;
    list.appendChild(chip);

    closeEmpPicker();
    showToast(`✅ ${res.name} added`, 'success');
  } catch (err) { showToast('❌ ' + err.message, 'error'); }
}
...
// Enter key in add-person inputs (engineers only; team uses picker)
document.addEventListener('keydown', e => {
  if (e.key !== 'Enter') return;
  const t = e.target;
  if (t.matches('#eng-name, #eng-role')) addPerson('engineer');
  if (t.matches('#empPickerSearch'))     { /* search already handled by oninput */ }
});
